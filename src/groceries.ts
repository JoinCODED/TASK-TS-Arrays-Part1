...
function getLastGroceryItem(groceries: string[]): string {
  // write your code here...
  const lastElement: string = groceries[groceries.length - 1];
...
function removeLastGroceryItem(groceries: string[]): string {
  ...
  return removedElement; // replace empty string with what you see is fit
}
...
function getFirstThreeGroceryItems(groceries: string[]): string[] {
  // write your code here...
  const threeElements: string[] = groceries.slice(0, 3);
  return threeElements; // replace empty array with what you see is fit
}
